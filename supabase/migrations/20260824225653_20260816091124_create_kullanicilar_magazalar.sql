...
- `kullanicilar`: Her kullanıcı yalnızca kendi satırını görebilir/ekleyebilir/güncelleyebilir/silebilir.
- `magazalar`: Her kullanıcı yalnızca kendi mağazalarını yönetebilir.
- `kullanici_id` sütunu `DEFAULT auth.uid()` ile tanımlıdır.
*/
...
CREATE TABLE IF NOT EXISTS magazalar (
  ...
  api_secret text,
  supplier_id text,
  sirket_turu text NOT NULL DEFAULT 'sahis',
